...
const assetSchema = z.object({
  ...
  serialNumber: z.string().min(4, "Serial number is too short"),
  tagId: z.string().startsWith("VIQ-", "Tag ID must start with VIQ-"),
...
function AddAssetModal({ onClose, onSuccess }: { onClose: () => void; onSuccess: () => void }) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const parsedData = assetSchema.parse({
        ...form,
      ...
      <div className="modal-card glass" onClick={e => e.stopPropagation()}>
        ...
        {!showManualForm ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', alignItems: 'center', padding: '20px 0' }}>
            <div style={{ width: '80px', height: '80px', background: 'rgba(0, 230, 118, 0.1)', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <Search size={32} style={{ color: 'var(--accent-success)' }} />
            </div>
            <p style={{ textAlign: 'center', color: 'var(--text-secondary)' }}>
              Scan the manufacturer barcode or NFC tag to automatically fetch asset metadata and register it instantly.
            </p>
            <button type="button" className="btn btn-primary" style={{ width: '100%', padding: '12px' }} onClick={() => {
              // Simulate scan
              setForm({ ...form, modelName: 'Dell XPS 15 (Scanned)', serialNumber: 'SN-SCANNED123', tagId: `VIQ-${Math.floor(Math.random() * 10000)}` });
              setShowManualForm(true);
            }}>
              Initialize Scanner
            </button>
            <button type="button" className="btn btn-outline" style={{ border: 'none', color: 'var(--text-muted)' }} onClick={() => setShowManualForm(true)}>
          ...
          <form onSubmit={handleSubmit} className="modal-form">
            ...
            <div className="modal-actions">
              ...
              <button type="submit" className="btn btn-primary glow-border">Register Asset</button>
            </div>
          </form>
        )}
      </div>
